...
const VoterRegistration = () => {
  ...
  const handleVerifyOtp = async (e) => {
    e.preventDefault();
    try {
      await axios.post("http://localhost:5000/api/users/send_otp", { aadhar_no: aadhaarNumber, otp });
      showToast("OTP Verified!", "success");

      await ElectionInstance.methods.registerVoter(name, aadhaarNumber).send({ from: account, gas: 1000000 });
      ...
      fetchVoterStatus(ElectionInstance, account);
    } catch (error) {
